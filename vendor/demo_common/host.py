...
class DemoStorefront(Protocol):
    ...
    ``StorefrontBackend`` interface it implements: the loaded catalog, the store name,
    per-session cleanup, and the cross-user order feed the merchant overview shows."""
...
async def _lifespan(_: FastAPI) -> AsyncIterator[None]:
    if not (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")):
        logger.info(
            "No API key in the environment or .env files; the Anthropic SDK falls back to "
            "its own credential chain. If chat returns auth errors, set ANTHROPIC_API_KEY "
            "in a .env file (repo root or the example's directory)."
        )
    yield
